...
let counter = 1 //счетчик для задания id для каждой таски
...
function createTask(objectData){ ///создание таски
    const root = document.createElement('div')///строка таски, на котором будет сам текст, чекбокс и кнопка удаления
    ...
    root.addEventListener('click', () => changeStatus(objectData.id)); //при нажатии на таску менятся ее статус, сделано/не сделано

    root.appendChild(chekinput)
    root.appendChild(txt)
    root.appendChild(btn)

    return root ///возвращаем готовую таску
...
function changeStatus(id) { //обновляет статус задачи
    ...
    if (task) {
        task.isDone = !task.isDone; //изменение статуса таски (isDone) на противоположный
        syncData()
        render()
    }
}
...
function deleteById(id) { /*Функция принимает идентификатор задачи (id) и ищет индекс этой задачи в массиве data с помощью метода findIndex. Если задача найдена, возвращается её индекс, иначе возвращается -1.*/
...
btnEl.addEventListener('click', ()=>{ ///что происходит при нажатии Добавить:
    ...
    // const taskEl = createTask(textValue)///прогоняет полученный текст через нашу функцию, теперь taskEl содержит в себе одну таску
    // listEl.appendChild(taskEl)//добавляет таску в основной лист задач
        syncData()
        render()
        inputEl.value=''
        // render()
